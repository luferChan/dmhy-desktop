...
- **多源搜索** — 动漫花园（支持「动漫 / 日剧 / RAW」等分类筛选）+ 新增蜜柑计划（`mikanani.me`）。搜索栏一键切换。字幕组前缀（`[字幕组]`）筛选解析自标题，蜜柑和动漫花园两个源都生效。
- **番组周表视图（蜜柑）** — 空关键词时进入按播出星期分块的番剧海报网格（周日 ~ 周六 + 剧场版 / OVA），点击番剧海报查看该番剧全量字幕组的发布。
- **一键下载** — 优先使用 `.torrent` 文件下载（速度快），自动回落 Magnet 链接。可选在主任务完成后自动清理中间的 `.torrent` 文件（下载对话框开关，默认开启）。
